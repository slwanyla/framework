...
export class SampaitujuanPage implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.route.queryParams.subscribe(params => {
      ...
      this.promoDiskon = +params['promoDiskon'] || 0;
  ...
  startPollingStatus() {
    this.pollingSubscription = interval(10000)
      .pipe(
        switchMap(() => this.http.get<{status: string}>(`https://api.example.com/order/status/${this.orderId}`))
      )
      .subscribe(
        async (res) => {
          if(res.status === 'sampai_tujuan' && !this.sudahNotified){
            this.sudahNotified = true;
