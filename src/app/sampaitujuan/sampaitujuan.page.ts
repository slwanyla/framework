...
export class SampaitujuanPage implements OnInit, OnDestroy {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      if (this.orderId) {
        this.http.get<any>(`https://localhost:8000/api/update-status/${this.orderId}`)
          .subscribe(order => {
            this.waktuSelesai = order.waktu_selesai
      ...
      console.log('Waktu selesai:', this.waktuSelesai);
  ...
  openChat() {
    this.router.navigate(['/chat'], { queryParams: { from: 'sampaitujuan' } });
  }
}
